...
function GMDashboardInner({ encounter = {}, tokens = [], players = {}, pushUpdate, hardResetSession }) {
    ...
    const deployEnemy = () => {
        ...
        pushUpdate(s => {
            ...
            const currentEnemies = safeArray(enc.enemies);
            return {
                ...s,
                encounter: {
                    ...enc,
                    enemies: [...currentEnemies, newHostile]
                }
            };
        });
    };
    ...
    const removeEnemy = (uid) => {
        ...
        pushUpdate(s => {
            const enc = s.encounter || {};
            const currentEnemies = safeArray(enc.enemies).filter(e => e && String(e.uid) !== String(uid));
    ...
    const advanceRound = () => {
        pushUpdate(s => {
            const enc = s.encounter || {};
            const nextRound = (enc.round || 0) + 1;
            let currentEnemies = safeArray(enc.enemies);

            currentEnemies = currentEnemies.map(e => {
                if (e && !e.isActive && e.spawnMode === 'round' && e.spawnRound === nextRound) {
    ...
    const nextTurn = () => {
        pushUpdate(s => {
            const enc = s.encounter || {};
            const q = safeArray(enc.initiativeQueue);
            if (q.length === 0) return s;
            let currentIdx = q.indexOf(enc.activeTokenId);
            let nextId = q[(currentIdx + 1) % q.length];
            return { ...s, encounter: { ...enc, activeTokenId: nextId } };
        });
    };
    ...
    const prevTurn = () => {
        pushUpdate(s => {
            ...
            if (q.length === 0) return s;
            let currentIdx = q.indexOf(enc.activeTokenId);
            let prevId = currentIdx > 0 ? q[currentIdx - 1] : q[q.length - 1];
            return { ...s, encounter: { ...enc, activeTokenId: prevId } };
        });
    };

    const setActiveToken = (tokenId) => {
        pushUpdate(s => ({ ...s, encounter: { ...(s.encounter || {}), activeTokenId: tokenId } }));
    };

    const moveInQueue = (idx, dir) => {
        pushUpdate(s => {
            const enc = s.encounter || {};
            let q = [...safeArray(enc.initiativeQueue)];
            if (idx + dir >= 0 && idx + dir < q.length) {
                const temp = q[idx];
                q[idx] = q[idx + dir];
                q[idx + dir] = temp;
            }
            return { ...s, encounter: { ...enc, initiativeQueue: q } };
        });
    };
                ...
                <div className="space-y-6">
                    <div className="bg-black border border-gray-700 p-5 space-y-4">
                        ...
                        <div className="flex justify-between items-center bg-gray-900 p-3 border border-gray-800">
                            <div>
                                <div className="text-xs text-gray-400 uppercase">Current Phase</div>
                                <div className="text-lg font-bold text-white">{safeEnc.round === 0 ? 'Deployment Phase' : `Round ${safeEnc.round}`}</div>
                            </div>
                        ...
                        <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
                            ...
                            {safeArray(safeEnc.initiativeQueue).map((tokenId, idx) => {
                                const t = safeArray(tokens).find(tk => tk && tk.id === tokenId);
                                if (!t) return null;
                                const isAct = safeEnc.activeTokenId === tokenId;
                        ...
                        <button className="w-full bg-[#ff6600] text-black font-bold px-4 py-3 uppercase text-xs tracking-wider hover:bg-white transition-colors mt-2" onClick={advanceRound}>
                            {safeEnc.round === 0 ? '▶ Deploy & Start Encounter' : 'Shuffle & Advance Round [+]'}
                        </button>
                    </div>
                    ...
                    <div className="bg-black border border-gray-700 p-5">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                            {enemies.length === 0 ? (
                                <div className="text-xs text-gray-600 italic">No hostiles staged.</div>
                            ) : (
                                enemies.map(e => e && (
                                    <div key={e.uid} className={`p-2.5 border flex justify-between items-center text-xs ${e.isActive ? 'bg-gray-900 border-[#ff6600]' : 'bg-black/50 border-gray-800 opacity-60'}`}>
                                        <div>
                                            <div className="font-bold text-white">{e.name} <span className="text-[10px] text-gray-500">[T{e.tier}]</span></div>
                                            <div className="text-[10px] text-gray-400">HP: {e.currentHp}/{e.maxHp} | {e.isActive ? 'Active' : `Delayed (${e.spawnMode})`}</div>
                                        </div>
                                        <button className="text-gray-500 hover:text-red-500 font-bold px-2" onClick={() => removeEnemy(e.uid)} title="Purge Hostile">✕</button>
                                    </div>
                                ))
                            )}
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-3">
                            ...
                            <select className="w-full bg-gray-900 border border-gray-700 p-2.5 text-white text-xs outline-none cursor-pointer" value={selectedBestiaryId} onChange={e => setSelectedBestiaryId(e.target.value)}>
                                {filteredBestiary.map(e => (
                                    <option key={e.id} value={e.id}>[Tier {e.tier}] {e.name} ({e.affinity} - {e.hp} HP)</option>
                                ))}
                            </select>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <select className="w-full bg-gray-900 border border-gray-700 p-2 text-white text-xs outline-none cursor-pointer" value={spawnMode} onChange={e => setSpawnMode(e.target.value)}>
                                        <option value="immediate">Immediate Drop</option>
                                        <option value="clear">Deploy On Clear</option>
                                        <option value="round">Deploy On Round X</option>
                                    </select>
                                </div>
                                {spawnMode === 'round' ? (
                                    <div>
                                        <label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Target Round</label>
                                        <input type="number" min="1" className="w-full bg-gray-900 border border-gray-700 p-2 text-white text-xs outline-none text-center font-bold" value={spawnRound} onChange={e => setSpawnRound(safeInt(e.target.value))} />
                                    </div>
                                ) : (
                                    <div className="flex items-end">
                                        <div className="text-[10px] text-gray-500 italic pb-2">Hostile ready for encounter insertion.</div>
                                    </div>
                                )}
                            </div>

                            <button className="w-full bg-[#ff6600] text-black font-bold p-3 uppercase text-xs tracking-wider hover:bg-white transition-colors mt-2" onClick={deployEnemy}>
                                + Stage Hostile to Encounter
                            </button>
                        </div>
                                ...
                                return (
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                                            ...
                                            <span className="bg-black px-2 py-0.5 border border-gray-700 text-[#ff6600] font-bold">Tier {template.tier}</span>
                                        </div>
                                        <div className="grid grid-cols-2 gap-2 text-gray-400">
                                            <div>Affinity: <span className="text-white font-bold">{template.affinity}</span></div>
                                            <div>Base HP: <span className="text-white font-bold">{template.hp}</span></div>
                                            <div>Barriers: <span className="text-white font-bold">{template.barriers?.length ? template.barriers.join(', ') : 'None'}</span></div>
                                            <div>Abilities: <span className="text-white font-bold">{template.abilities?.length || 0}</span></div>
                                        </div>
                                        <div className="border-t border-gray-800 pt-2">
                                            <div className="text-[10px] text-gray-500 uppercase tracking-widest mb-1 font-bold">Ability Registry:</div>
                                            <div className="space-y-1 max-h-32 overflow-y-auto">
                                                {safeArray(template.abilities).map((ab, idx) => {
                                                    const abName = typeof ab === 'object' ? ab.name : String(ab).split(':')[0];
                                                    const abCost = typeof ab === 'object' ? ab.cost : 1;
                                                    return <div key={idx} className="text-gray-300 truncate">• {abName} <span className="text-gray-500">({abCost} Res)</span></div>;
                                                })}
                                            </div>
                                        </div>
                                    </div>
