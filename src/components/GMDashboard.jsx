...
export default function GMDashboard({ encounter = {}, pushUpdate }) {
    ...
    const addEnemy = (bestiaryId) => {
        ...
        const newEnemy = { 
            ...
            currentBarriers: template.barriers ? [...template.barriers] : [],
            siphonActive: false,
            staggered: false
        };
    ...
    const isOverload = (encounter?.round || 1) >= 4;
            ...
            <div className={`p-4 border ${isOverload ? 'bg-red-950 border-red-500' : 'bg-[#1a222c] border-slate-700'}`}>
                ...
                <div className="text-center mb-6">
                    ...
                    <div className="flex gap-2">
                        <button className="flex-1 bg-gray-800 border border-gray-600 p-2 hover:bg-gray-700 text-white" onClick={() => updateEnc({ round: Math.max(1, (encounter?.round || 1) - 1) })}>- Prev</button>
                        <button className="flex-1 bg-[#00f0ff] text-black font-bold p-2 hover:bg-white transition-colors" onClick={() => updateEnc({ round: (encounter?.round || 1) + 1 })}>Next +</button>
                    </div>
                </div>
            ...
            <div className="lg:col-span-3 bg-[#1a222c] p-4 border border-slate-700 flex flex-col h-[75vh]">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                    {enemiesList.length === 0 && (
                        <div className="text-gray-500 text-center mt-10 border border-dashed border-gray-700 p-8">No active entities on the grid.</div>
                    )}
                        ...
                        <div key={enemy.uid} className={`border p-3 flex flex-col md:flex-row md:justify-between items-start md:items-center gap-4 ${enemy.staggered ? 'border-yellow-500 bg-yellow-900 bg-opacity-20' : 'border-gray-700 bg-black'}`}>
                            <div className="flex-1 w-full">
                                <div className="font-bold text-white text-lg flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-xs bg-gray-800 px-2 py-0.5 text-gray-400 border border-gray-600">T{enemy.tier || 1}</span>
                            ...
                            <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                                <button 
                                    ...
                                    onClick={() => {
                                        ...
                                        newEnemies[idx] = { ...newEnemies[idx], siphonActive: !newEnemies[idx].siphonActive };
                                    ...
                                    <div key={bIdx} className="text-center shrink-0">
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const newEnemies = [...enemiesList];
                                                const updatedEnemy = { ...newEnemies[idx] };
                                                const updatedBarriers = [...(updatedEnemy.currentBarriers || [])];
                                                
                                                updatedBarriers[bIdx] = val;
                                                updatedEnemy.currentBarriers = updatedBarriers;
                                                
                                                if (val <= 0 && bar > 0) updatedEnemy.staggered = true;
                                                
                                                newEnemies[idx] = updatedEnemy;
                                                updateEnc({ enemies: newEnemies });
                                            }} 
                                        />
                                    </div>
                                ...
                                <div className="text-center shrink-0">
                                    ...
                                    <input 
                                        ...
                                        onChange={(e) => {
                                            const val = parseInt(e.target.value) || 0;
                                            const newEnemies = [...enemiesList];
                                            newEnemies[idx] = { ...newEnemies[idx], currentHp: val };
                                            updateEnc({ enemies: newEnemies });
                                        }} 
                                    />
                                </div>
