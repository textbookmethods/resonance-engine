...
const STATE_DESCRIPTIONS = { 'Hijacked': 'Unit is controlled by opposing network.', 'Execute': 'Targeted for immediate elimination.', 'Bleed': 'Sustaining physical hemorrhaging over time.', 'Burn': 'Engulfed in thermal plasma.', 'Poisoned': 'Toxic decay eroding biological systems.', 'Immobilized': 'Locomotion locked down.', 'Stunned': 'Neural feedback loop paralyzing actions.', 'Shielded': 'Energy barrier mitigating incoming payloads.', 'Vulnerable': 'Armor integrity compromised (1.5x damage).', 'Knockdown': 'Knocked prone on the tactical grid.', 'Blind': 'Targeting optics obscured (Range limited to 1).', 'Haste': 'Locomotion velocity enhanced (+2 Move).', 'Slowed': 'Thermal chilling reducing velocity (-2 Move).', 'Shocked': 'Electrical interference disrupting circuitry.', 'Evasive': 'Trajectory blurred, evading incoming vectors.', 'Invulnerable': 'Stasis field completely negating damage.' };
...
function GridBoardInner({ players = {}, grid = [], tokens = [], encounter = {}, activeAction = null, pushUpdate, role, localId }) {
    ...
    const evaluateCrush = (tokensList, currentGrid, playersObj, enemiesList, deadEnemyUids, logStr) => {
        ...
        safeArray(tokensList).forEach(t => {
            if (!t) return; const crushed = isCrushed(t.pos, currentGrid);
    ...
    const advanceTurn = () => {
        pushUpdate(s => {
            ...
            const newTokens = deepClone(safeArray(s.tokens)); const pClone = deepClone(s.players || {});
            const nTIdx = newTokens.findIndex(t => t && t.id === nextId);
            if (nTIdx !== -1) {
                newTokens[nTIdx].movementRemaining = newTokens[nTIdx].speed ?? 3;
                if (newTokens[nTIdx].type === 'player') { const pId = newTokens[nTIdx].refId; if(pClone[pId]) { pClone[pId].usedBasicAttack = false; pClone[pId].usedParry = false; pClone[pId].usedEvade = false; } }
            }
    ...
    const primeCard = (c, isImprovised = false, originalCost = 0, targetTokenOverride = null) => {
        ...
        let finalRange = isBlind ? '1' : (c.range || '1'); let finalAoe = isBlind ? 0 : (c.a || 0); if (isBlind) alert("Warning: BLIND state active. Targeting optics restricted to adjacent hexes and AoE is zeroed.");
        ...
        pushUpdate(s => ({ ...s, activeAction: { type: isBlink ? 'blink' : 'target', source: String(p.name || 'Player'), sourceId: String(activeT.refId), isEnemy: false, isBasic: false, isImprovised: isImprovised || false, originalCost: safeInt(originalCost), cost: safeInt(requiredRes), name: String(c.name || 'Custom Action'), payload: String(c.payload || 'damage'), d: safeInt(c.d), a: finalAoe || 0, u: safeInt(c.u), m: safeInt(c.m), coreMobility: String(coreMobility || ''), range: String(finalRange || '1'), effectName: String(c.effectName || ''), effectCore: String(c.effectCore || getCoreState(c.effectName) || ''), elementRaw: String(c.elementRaw || 'Kinetic'), elementCore: String(c.elementCore || getCoreElement(c.elementRaw || 'Kinetic')), terrain: String(c.terrain || ''), desc: String(c.desc || ''), cardId: c.id } })); 
    };

    const archiveEquippedCard = (card, targetTokenOverride = null) => {
        const activeT = targetTokenOverride || activeTokens.find(t => t && t.id === selectedToken); if (!activeT) return;
        const p = safePlayers[activeT.refId] || {}; const archived = safeArray(p.savedSkills); if (archived.some(s => s && String(s.name).toLowerCase() === String(card.name).toLowerCase())) return alert(`"${card.name}" is already archived in your Spellbook.`);
        pushUpdate(s => { const pClone = deepClone(s.players || {}); if (pClone[activeT.refId]) { pClone[activeT.refId].savedSkills = [...safeArray(pClone[activeT.refId].savedSkills), card]; pClone[activeT.refId].customCards = safeArray(pClone[activeT.refId].customCards).filter(item => item && String(item.id) !== String(card.id)); } return { ...s, players: pClone }; }); alert(`"${card.name}" archived to Spellbook!`);
    };
    ...
    const executeMove = (index) => {
        ...
        pushUpdate(s => {
            ...
            if (tIdx !== -1) {
                ...
                let finalPlayers = deepClone(s.players || {}); const entName = t.type === 'player' ? String(finalPlayers[t.refId]?.name || 'Agent') : String(activeEnemies.find(e=>e.uid === t.refId)?.name || 'Hostile'); const logEntry = { id: Date.now() + Math.random(), text: `>> MOVEMENT: ${entName} repositioned to Hex ${index}.` }; let newLogFeed = [...safeArray(s.encounter?.logFeed), logEntry].slice(-30);
                if (t.type === 'player') { const res = evaluateCrush(newTokens, getSafeGrid(s.grid), finalPlayers, s.encounter?.enemies || [], new Set(), ""); if (res.logStr) { finalPlayers = res.pObj; newLogFeed.push({ id: Date.now() + Math.random(), text: String(res.logStr) }); } }
        ...
        pushUpdate(s => {
                ...
                if (aoeHexes.includes(t.pos)) {
                    ...
                    if (t.type === 'enemy') {
                        ...
                        if (p) {
                            ...
                            else if (payloadType === 'battery') { let pRes = p.resPool ?? 3; p.resPool = Math.min(10, pRes + rawDmg); log += `Agent [${p.name || 'P1'}]: Energized for +${rawDmg} Resonance.\n`; if (action.sourceId && String(t.refId) !== String(action.sourceId)) triggeredAssist = true; fctQueue.push({ pos: t.pos, text: `+${rawDmg} RES`, color: '#00f0ff', id: Math.random() }); } 
                            else {
                                if (rpsMult === 1.5) { incomingDmg = Math.ceil(incomingDmg * 1.5); log += `>> AFFINITY ADVANTAGE: 1.5x Dmg\n`; } if (rpsMult === 0.5) { incomingDmg = Math.ceil(incomingDmg * 0.5); log += `>> AFFINITY DISADVANTAGE: 0.5x Dmg\n`; } if (isFlanking) { incomingDmg = Math.ceil(incomingDmg * 1.5); log += `>> FLANKING BONUS: 1.5x Dmg\n`; }
                                if (isOnSteamReact) { incomingDmg += 5; log += `>> STEAM BLAST: (+5 Dmg)\n`; } if (isOnCombustReact) { incomingDmg += 5; log += `>> COMBUSTION: (+5 Dmg)\n`; } if (isOnConductReact) { incomingDmg += 5; log += `>> CONDUCTION: (+5 Dmg)\n`; } if (isOnAnnihilateReact) { incomingDmg += 5; log += `>> ANNIHILATION: (+5 Dmg)\n`; }
                                if (coreStates.includes('Vulnerable')) incomingDmg = Math.ceil(incomingDmg * 1.5); if (coreStates.includes('Shielded')) { incomingDmg = Math.max(0, incomingDmg - 5); log += `>> [Shielded] mitigated 5 damage.\n`; } if (coreStates.includes('Invulnerable')) { incomingDmg = 0; log += `>> [Invulnerable] completely negated the attack.\n`; }
            ...
            if (action.isEnemy && !action.isHijacked) { newEnemyPoolTotal = Math.max(0, newEnemyPoolTotal - safeInt(action.cost)); log += `>> [-${safeInt(action.cost)} Res] Hostile Action executed.\n`; } 
                ...
                if (p) {
                    ...
                    else { const costDeduction = safeInt(action.cost); pRes = Math.max(0, pRes - costDeduction); if (costDeduction > 0) log += `>> [-${costDeduction} Res] Skill executed.\n`; if (action.cardId) { const cIdx = safeArray(p.customCards).findIndex(c => c && String(c.id) === String(action.cardId)); if (cIdx !== -1) { const usedCard = p.customCards.splice(cIdx, 1)[0]; p.savedSkills = safeArray(p.savedSkills); if (!p.savedSkills.some(s => s && String(s.name).toLowerCase() === String(usedCard.name).toLowerCase())) { p.savedSkills.push(usedCard); } log += `>> SYSTEM ROUTING: [${usedCard.name}] purged from active HUD and returned to Spellbook.\n`; } } }
                    let syncLogs = []; if (triggeredExploit) { pRes += 2; syncLogs.push('EXPLOIT (+2 Res)'); } if (triggeredTagTeam) { pRes += 2; syncLogs.push('TAG-TEAM (+2 Res)'); } if (triggeredAssist) { pRes += 1; syncLogs.push('ASSIST (+1 Res)'); } if (syncLogs.length > 0) log += `>> AUTOMATED SYNERGY: ${syncLogs.join(', ')}\n`; p.resPool = Math.min(10, pRes);
                }
            }
            ...
            if (deadEnemyUids.size > 0) { newEnemies = newEnemies.filter(e => e && !deadEnemyUids.has(String(e.uid))); newTokens = newTokens.filter(t => t && !(t.type === 'enemy' && deadEnemyUids.has(String(t.refId)))); }
    ...
    const renderInitiativeTracker = () => {
        ...
        return (
            <div className="absolute top-2 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-2 bg-black/80 border border-gray-700 p-2 shadow-lg max-w-full overflow-x-auto scrollbar-hide">
                ...
                <button onClick={advanceTurn} className="ml-4 bg-gray-800 text-white text-[10px] font-bold px-3 py-1.5 uppercase border border-gray-600 hover:bg-white hover:text-black transition-colors whitespace-nowrap">Next Turn ↷</button>
            </div>
        );
    };
    ...
    const renderTargetPreviews = () => {
        ...
        const payloadType = activeAction.payload || 'damage'; const rawDmg = safeInt(activeAction.d); const dispCore = activeAction.elementCore || activeAction.elementRaw || 'Kinetic'; const isExecute = activeAction.effectCore === 'Execute';
        return activeTokens.map(t => {
            ...
            if (isEnemy) { const e = safeArray(safeEnc?.enemies).find(en => en && String(en.uid) === String(t.refId)); if (e) { entAffinity = e.affinity; entStates = safeArray(e.statuses).map(st => getCoreState(st)); } } else { pObj = safePlayers[t.refId]; if (pObj) { entAffinity = pObj.affinity; entStates = safeArray(pObj.statuses).map(st => getCoreState(st)); } }
            ...
            if (payloadType === 'damage') {
                ...
                } else if (isEnemy) { const e = safeArray(safeEnc?.enemies).find(en => en && String(en.uid) === String(t.refId)); if (e) { let barriers = safeArray(e.currentBarriers).reduce((a,b) => a+b, 0); if (barriers > 0) { mitigation = barriers; mitName = "Barriers"; } finalDmg = Math.max(0, incomingDmg - barriers); } }
            ...
            return (
                <div key={`preview-${t.id}`} className="absolute z-[120] bg-black/95 border border-[#00f0ff] p-2 flex flex-col text-[10px] w-48 shadow-[0_0_15px_rgba(0,240,255,0.4)] pointer-events-none transition-all duration-200" style={{ left: `${x + hexWidth + 5}px`, top: `${y - 10}px` }}>
                    <div className="text-[#00f0ff] font-bold border-b border-gray-700 pb-1 mb-1 uppercase flex justify-between"><span>Target Math</span><span>{t.type === 'player' ? String(pObj?.name || 'Agent') : 'Hostile'}</span></div>
    ...
    const renderHexBackgrounds = () => {
        ...
        return activeGrid.map((cell, idx) => {
            ...
            return ( <div key={`bg-${idx}`} onClick={() => handleHexClick(idx)} onMouseEnter={() => setHoveredHex(idx)} onMouseLeave={() => setHoveredHex(null)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleHexDrop(e, idx)} className="absolute transition-all duration-150" style={{ zIndex: hexZ, left: `${x}px`, top: `${y}px`, width: `${hexWidth}px`, height: `${hexHeight}px`, backgroundColor: bgColor, border: hexBorder, clipPath: 'polygon(25% 0%, 75% 0%, 100% 50%, 75% 100%, 25% 100%, 0% 50%)', transform: 'scale(0.95)', cursor: activeAction && activeAction.type !== 'hijack_select' ? 'crosshair' : 'pointer' }}>{isBlockedByLoS && !isAoETarget && showTargetMask && <div className="absolute inset-0 flex items-center justify-center text-red-500 opacity-20 text-[10px]">✕</div>}</div> );
        });
    };
    ...
    const renderTokens = () => {
        ...
        return activeTokens.map(t => {
            ...
            let displayChar = t.type === 'enemy' ? 'E' : 'P'; let tBg = t.type === 'enemy' ? '#ff6600' : '#00f0ff'; let txtColor = '#000000';
            ...
            if (activeT.type === 'player') {
                ...
                return (
                    <div className="w-full md:w-64 bg-[#1a222c] p-4 border font-mono flex flex-col gap-3 shrink-0 h-full overflow-y-auto" style={{ borderColor: pColor }}>
                        {isThisActiveToken && (isGM || amIOwner) && safeEnc.round > 0 && (
                            <button className="w-full bg-[#22c55e] text-black font-bold py-3 text-sm uppercase tracking-widest hover:bg-white transition-colors border-2 border-[#22c55e] animate-pulse mb-2" onClick={(e) => { e.stopPropagation(); advanceTurn(); }}>
                                End Turn
                            </button>
                        )}
                            ...
                            return (
                                <div key={c.id || Math.random()} className="bg-black border border-[#00f0ff] p-2 text-xs relative group flex flex-col">
                                    <div className="flex-1 pr-6 pb-2">
                                        ...
                                        <button className={`mt-auto w-full font-bold py-1 uppercase transition-colors ${(isNoFuel || disableAttacks || !isMyTurn) ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-gray-800 text-white hover:bg-white hover:text-black'}`} disabled={isNoFuel || disableAttacks || !isMyTurn} onClick={() => primeCard(c, false, 0, activeT)}>
            ...
            if (activeT.type === 'enemy') {
                ...
                return (
                    <div className="w-full md:w-64 bg-[#1a222c] p-4 border border-[#ff6600] font-mono flex flex-col gap-3 shrink-0 h-full overflow-y-auto">
                        {isThisActiveToken && isGM && safeEnc.round > 0 && (
                            <button className="w-full bg-[#ff6600] text-black font-bold py-3 text-sm uppercase tracking-widest hover:bg-white transition-colors border-2 border-[#ff6600] animate-pulse mb-2" onClick={(e) => { e.stopPropagation(); advanceTurn(); }}>
                                End Turn
                            </button>
                        )}
                        ...
                        <div className="bg-gray-900 border border-gray-700 p-2 mt-1">
                            ...
                            {isGM && (
                                <div className="flex gap-1">
                                    <select id="eState" className="flex-1 bg-black border border-gray-600 text-white text-[10px] p-1 outline-none">
                                        <option value="">-- Add State --</option>
                                        {Object.keys(STATE_DESCRIPTIONS).map(st => <option key={st} value={st}>{st}</option>)}
                                    </select>
                                ...
                                <div key={aIdx} className="bg-gray-900 border border-gray-700 p-2 text-sm flex justify-between items-center relative">
                                    ...
                                    {isGM && (
                                        <button className={`font-bold px-2 py-1 uppercase text-[10px] border transition-colors ${disableAttacks ? 'bg-gray-800 text-gray-500 border-gray-600 cursor-not-allowed' : 'bg-gray-800 text-white hover:bg-[#ff6600] hover:text-black border-gray-600'}`} disabled={disableAttacks} onClick={() => {
                                            ...
                                            const currentHostileRes = safeEnc?.enemyPoolTotal ?? 10; if (currentHostileRes < ab.cost) return alert(`System Locked: Insufficient Hostile Resonance.\nRequired: ${ab.cost} RES\nCurrent Pool: ${currentHostileRes} RES`);
    ...
    const renderFCT = () => {
        return floatingTexts.map(fct => {
            ...
            return (
                <div key={fct.id} className="absolute z-[300] font-bold text-2xl pointer-events-none fct-anim whitespace-nowrap drop-shadow-xl" style={{ left: `${x + hexWidth/2}px`, top: `${y}px`, color: fct.color, textShadow: '0px 0px 8px rgba(0,0,0,1), 0px 0px 4px rgba(0,0,0,1)' }}>
                    {fct.text}
                </div>
            );
        });
